#include "utils.h"

//------------------------------------------
//DESCRIPTION:RTU CRC 校验的高位字节表
//------------------------------------------
BYTE auchCRCHis[260] = {
    
} ;
//------------------------------------------
//DESCRIPTION:RTU CRC 校验的低位字节表
//------------------------------------------
BYTE auchCRCLos[260] = {
    
} ;

QString byteToHexStr(BYTE *b, int length)
{
    QString stmp="";

    for (int n=0;n<length;n++)
    {
        QString tmp = QString::number(b[n]&0xff,16);
        stmp.append((tmp.length()==1)? "0"+tmp : tmp);
    }
    return stmp.toUpper().trimmed();
}

BYTE *RTU_CRC(BYTE *crc, BYTE *puchMsg, int usDataLen)
{
    qDebug()<<"校验数据："<<byteToHexStr(puchMsg,usDataLen);
    qDebug()<<"校验长度："<<usDataLen;
    BYTE uchCRCHi = 0xFF ; //* 高CRC字节初始化
    BYTE uchCRCLo = 0xFF ; //* 低CRC 字节初始化
    int uIndex ; // CRC循环中的索引
    while (usDataLen--) // 传输消息缓冲区
    {
        BYTE b = (BYTE)uchCRCHi ^ (BYTE)(*puchMsg++); // 计算CRC
        uIndex = b&0x000000FF;
        uchCRCHi = (BYTE)(uchCRCLo ^ (BYTE)auchCRCHis[uIndex]) ;
        uchCRCLo = (BYTE)auchCRCLos[uIndex] ;
    }
    qDebug()<<"校验位："<<QString::number(uchCRCLo&0xff,16)<<" "<<QString::number(uchCRCHi&0xff,16);
    qDebug()<<"ff";
    crc[0] = uchCRCLo;
    crc[1] = uchCRCHi;
    qDebug()<<"ss";
    return crc;
}
